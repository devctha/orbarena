(function () {
  ...
  class BattleLogger {
    ...
    reset() {
      this.entries = [];
      this.stats = { player: emptyStats(), enemy: emptyStats() };
    }

    logDamage(time, attacker, target, damage, critical, source = "collision", abilityId = null) {
      const attackStats = this.stats[attacker.team];
      const targetStats = this.stats[target.team];
      ...
      if (this.entries.length > 260) this.entries.shift();
    }

    logAbility(time, fighter, ability) {
      const stats = this.stats[fighter.team];
    ...
    buildFighterResult(world, fighter) {
      ...
      return {
        ...
        mostUsedAbility: powerUse ? (OA.PowerRegistry.get(powerUse[0])?.name || powerUse[0]) : this.mostUsedAbility(fighter.team)
      };
    }
    ...
    buildResult(world, winner) {
      return {
        ...
        result: winner === "player" ? "Vitória" : "Derrota",
