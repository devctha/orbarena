(function () {
  ...
  class ControlSystem {
    ...
    request(world, fighter, slot, targetHint = null) {
      ...
      if (state?.castTime > 0) { state.casting = state.castTime; fighter.castQueue = { ability, target: target || fighter, delay: state.castTime };world.presentationSystem?.prepare(world,fighter,target||fighter,ability,state.castTime); world.events.push({ type: "castStart", fighter, ability: ability.id, duration: state.castTime }); return { ok: true, casting: true }; }
