(function () {
  ...
  class Game {
    ...
    buildSystems(seed, physics) {
      ...
      this.abilitySystem=new OA.AbilitySystem(this.random,this.projectiles,this.combat,this.particles,this.audio,this.logger); this.physics=new OA.PhysicsSystem(this.random,this.combat,this.projectiles);
    ...
    start(build, difficultyKey, seed, physicsSettings=OA.getPhysicsPreset("arcade"), options={}) {
      ...
      for(const fighter of world.fighters) this.initializeFighter(world,fighter,fighter._build||{},fighter===world.player?build:{});
    ...
    update(dt) {
      ...
      if(w.phase==="active"){
        ...
        this.musicSystem?.setState?.(w.battlePhase);this.musicSystem?.update?.(w,simDt); this.arenaSystem.update(w,simDt); this.controlSystem.update(w,simDt); this.characterSystem.update(w,simDt); this.summonSystem.update(w,simDt); this.cloneSystem.update(w,simDt); this.poisonSystem.update(w,simDt);
        for(const fighter of w.fighters) this.spikeSystem.update(fighter,simDt*(w.timeScales.fighter[fighter.team]||1));
        this.abilitySystem.update(w,simDt); this.perkSystem.update(w,simDt); this.weaponSystem.update(w,simDt); this.physics.update(w,simDt); this.projectiles.update(w,simDt); this.comboSystem.update(w,simDt); this.burstProtection.update(w,simDt); this.particles.update(simDt*(w.timeScales.effect||1)); this.replaySystem.capture(w,simDt);
        ...
        if(w.mode==="lab"&&winner){for(const fighter of w.fighters){if(!fighter.alive){fighter.alive=true;fighter.health=fighter.maxHealth;fighter.shield=0;const spawn=this.teamSystem.spawn(w.match.teams.findIndex((team)=>team.id===fighter.teamId),fighter.slot,w.match.teams.find((team)=>team.id===fighter.teamId)?.size||1,w.arena);fighter.x=spawn.x;fighter.y=spawn.y;fighter.vx=fighter.vy=0;fighter._eliminationRecorded=false;}}}
    ...
    lab(action){const w=this.world,f=w?.controlledFighter||w?.player,t=f&&OA.findTarget(w,f);if(!w||w.mode!=="lab"||!f)return;if(action==="heal")f.heal(f.maxHealth);if(action==="selfDamage")f.applyDamage(20,{source:"lab",ignoreArmor:true});if(action==="targetDamage"&&t)this.characterSystem.damage(w,f,t,20,"lab",false);if(action==="reset"){for(const id of Object.keys(f.abilityCooldowns))f.abilityCooldowns[id]=0;for(const state of Object.values(f.abilityState||{})){state.charges=state.maxCharges;state.rechargeTimer=0;}f.characterState.activeCooldown=0;}if(action==="ultimate"&&t){f.characterState.ultimateCharge=100;this.characterSystem.useUltimate(w,f,t,OA.CharacterMechanics.get(f.characterId));}if(action==="clone")this.cloneSystem.create(w,f,{life:12});if(action==="projectiles"&&t)this.characterSystem.burst(w,f,t,{radial:true,count:12,damage:6,bounces:2,color:f.glowColor});if(action==="status"&&t)this.poisonSystem.apply(t,f,4,8,2);if(action==="time"&&t)this.timeSystem.slowFighter(t.team,.25,5,f);}
